...
3. **Call sites — 6 keys, 7 sites** (defaultValue = current text byte-for-byte):
   ...
   Deliberately excluded: RateLimiter 500 `RateLimiter error` (ops-facing, not user language); Pagination schema message (no `req` in scope — parked with P1y). HttpServer 404/500 sinks were excluded from PR 1 but shipped later as a 5.4 rider (`http.notFound`/`http.serverError`, module-level translate in HttpServer.ts).
...
- **Example repo** (`framework-example-project`, AFTER the 5.4 npm release — rules must match the published version): bump dep to ^5.4.0 and **add `i18next` + `i18next-fs-backend` to its dependencies** (no longer transitive; the project genuinely translates); showcase `middleware.auth.notLoggedIn` override in en+ru; revisit its partial `auth.*` copies. **Update the AI-agent rules**: `.clinerules/01-ResponceType.md` + `03-ControllerPattern.md` teach `t('key') || 'fallback'` — always broken on a miss (t returns the truthy key), replace with `t('key', { defaultValue: '…' })` everywhere; `.clinerules/08-Internationalization.md` + `11-Testing.md` sweep for framework-key/i18next-bundled claims; `AGENTS.md` "tests assert raw keys" claim gets the 5.4 nuance (app keys raw, framework messages English). Docker-only verification.
