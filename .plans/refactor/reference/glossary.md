...
- **`CacheDriver`** — backend interface in `src/services/cache/CacheDriver.ts`: `get(key)`, `set(key, value, ttlSeconds)`, `del(key)`, optional `whenReady`. Drivers store already-serialized **strings** keyed by string and surface backend failures by **throwing** (the orchestrator catches + degrades); they don't namespace, dedup, serialize, or fail-soft themselves.
- **`MemoryDriver`** — default driver (`drivers/MemoryDriver.ts`): per-process `Map` with per-key `setTimeout` TTL (`.unref()`'d). Needs no external service — this default is what makes `@redis/client` optional. Per-process only (multi-node ⇒ redis).
- **`RedisDriver`** — `drivers/RedisDriver.ts`: lazy-`import()`s `redisConnection` (and thus `@redis/client`) inside `#init`, so the package only loads when `config('cache').driver === 'redis'`. The `import type { RedisClientType }` is erased at compile time. Each call resolves the live client through the shared single-flight helper.
- **`Cache` (orchestrator)** — `resolveDriver(config('cache').driver)` (a `CacheDriver` instance may be injected directly); owns namespacing, `promiseMapping` request dedup, JSON/bigint (de)serialization, fail-soft degradation, and the zero-TTL skip (#10). The cache key `namespace` lives in `config('redis')` (shared with the rate limiter, regardless of driver).

## Boot + extension (P1b)

- **Project boot hook** — a `bootHttp(app)` function passed **explicitly** to the `Server` constructor (`bootHttp` option, type `BootHttpHook`). Framework calls it during `startServer` after controllers register, before the adapter mounts. HTTP-specific (needs `app.httpServer`, which only exists once the server boots — CLI/worker processes never run it). Home for ad-hoc routes (`registerRoute`), Express middleware, and, later, global middleware. Wired in code, NOT file-discovered — every framework folder is already owned (`config/` merges its files, `controllers/` auto-loads its files), so there's no conflict-free folder to scan. Shipped 2026-06-22 (`BootHttpHook` type + the call inlined in `server.ts` — no separate module). _(Earlier attempts — a `controllers/index.ts` file with synthesized `loadAll()` default; then a magic file in `config/` or next to `controllers/` — were all dropped: they collided with the merge / auto-load folder schema.)_
