...
- **`HttpResponse` + `ResponseWriter` is the engine-neutral outbound seam (v5.3 / P1q).** Controllers return branded, typed descriptors for JSON/text/bytes/streams/files/redirects/empty/native-Web responses. `ExpressResponseWriter` renders them in v5.3; later adapters render the same values without changing consumers. A Web `Response` is a supported pass-through body, not the ordinary authoring type, because it erases body/status generics needed by codegen and OpenAPI.
...
- **Legacy `{validate, cast}` plain-object schemas removed.** Audit of 4 production codebases (~100 schemas) found zero usages. The legacy documentation section was already replaced with the Standard Schema validation guide. At `5.0.0-beta.45`, hard breaks are cheap. Bonus: users wrapping as Standard Schema get free TypeScript types via `InferOutput`.

- **Drop `req:` context in validators.** An audit of four production codebases (98 schemas total) found **zero** uses of `req` / `req.query` / `req.body` / `req.appInfo` inside `.test()` or `.when()` callbacks. The framework docs do not document `req:` context as a feature. Cross-field validation in user code lives within the schema's own data tree (yup's `this.parent`, zod's `.refine`); auth-aware logic belongs in the handler, not the schema.

- **Preserve yup `stripUnknown` behavior.** The same audit found eight production call sites that spread `req.appInfo.request` into `Model.create(...)` — losing implicit stripping would be a security regression. The yup driver always strips (matching today's behavior). Users who genuinely want unknowns can switch to zod with `.passthrough()`, valibot's `looseObject`, or register a custom driver.
...
- **Single-mount Express integration.** The framework calls `app.express.use(adapter)` exactly once. The adapter delegates path matching, method dispatch, parameter extraction, and middleware execution to our `RouteRegistry`. Express keeps HTTP lifecycle (`app.listen`), body parsing, and the third-party middleware ecosystem (`cors`, `helmet`, `compression`). Through v5.1 it also owns the response API (`res.json`, `res.status`); P1q/v5.3 adds a framework `ResponseWriter` while preserving those legacy calls. It stops being a router. Trade-off: ~50 lines of matcher in our code. Gains: `http.route` populated directly from the registry (no regex reverse-engineering for OTel); middleware spans named from `MiddlewareEntry.Class.name`; engine-neutral dispatch (Express adapter is swappable by re-implementing request conversion + response writing); future find-my-way swap-in is a `match()` replacement, not an adapter rewrite.
...
- **Per-route `bodyParsing: 'parsed' | 'raw' | 'none'`.** Routes opt out of parsing for raw-body scenarios (signed webhook verification, streaming uploads, custom binary protocols). `RequestParser` reads `req.routeMeta.bodyParsing`: `'parsed'` (default) dispatches by Content-Type to the parser registry (json/multipart/urlencoded); `'raw'` captures the request stream into `req.rawBody: Buffer` and skips parsing; `'none'` leaves the request untouched. Settable on `RouteNode` (inherits down the subtree, leaf-wins) and on `HandlerEntry` (leaf only). Default `'parsed'` preserves today's behavior in v5. This resolves a production workaround where a consumer replaced `RequestParserMiddleware.prototype.middleware` to capture signed webhook bytes.
...
- **`next()` and ordinary raw `res` are removed together in v6.** The v5.3 bridge keeps `(req, res, next)` compatible while allowing handlers to return `HttpResponse`. v6 registry middleware becomes `(ctx) => HttpResponse | void | Promise<...>`: return void to continue, return a response to stop, throw to enter the error registry. Ordinary controllers also stop receiving `res`. `HttpResponse.native(WebResponse)` remains portable; truly raw WebSocket/`CONNECT`/socket routes are separately declared and adapter-specific. Migration: framework ships a `legacyMiddleware(fn)` wrapper during the v6 transition; hard-removed by v7.
...
- **v5.3 introduces the response bridge additively.** Branded `HttpResponse` returns activate the writer; every non-branded return remains legacy behavior. Framework-owned handlers migrate first. A handler that both mutates `res` and returns `HttpResponse` fails loudly. v6 removes ordinary `res` only after the returned path has shipped and been exercised by consumers.
- **Named rate limits are a consumer config recipe, not a framework feature.** The application declares a `policy` object in its `rateLimiter` config, reads the final merged value with `app.getConfig('rateLimiter')`, and passes `policy.someName` as the existing middleware tuple parameters. `RateLimiter` already merges that object over its defaults; existing config-shape codegen provides autocomplete and typo checking. Rejected: framework-exported policy tokens (duplicate/capture config), `RateLimiter.policy('name')` (unnecessary string registry), and untyped `{ policy: 'name' }` parameters. No runtime implementation is planned unless a concrete need for JavaScript-side missing-policy validation appears.
