...
- **`HttpResponse` + `ResponseWriter` is the engine-neutral outbound seam (v5.2 / P1q).** Controllers return branded, typed descriptors for JSON/text/bytes/streams/files/redirects/empty/native-Web responses. `ExpressResponseWriter` renders them in v5.2; later adapters render the same values without changing consumers. A Web `Response` is a supported pass-through body, not the ordinary authoring type, because it erases body/status generics needed by codegen and OpenAPI.
...
- **`next()` and ordinary raw `res` are removed together in v6.** The v5.2 bridge keeps `(req, res, next)` compatible while allowing handlers to return `HttpResponse`. v6 registry middleware becomes `(ctx) => HttpResponse | void | Promise<...>`: return void to continue, return a response to stop, throw to enter the error registry. Ordinary controllers also stop receiving `res`. `HttpResponse.native(WebResponse)` remains portable; truly raw WebSocket/`CONNECT`/socket routes are separately declared and adapter-specific. Migration: framework ships a `legacyMiddleware(fn)` wrapper during the v6 transition; hard-removed by v7.
