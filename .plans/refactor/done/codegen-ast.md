...
**Status**: ✅ implemented (2026-06-09) — AST-only codegen front-end. `astExtract` (oxc) + `astResolve` (extends-walk/import-map) + `astSpec` (→ shared `buildSubtreeFromSpec`) + `astEmit` (flatten + render) + `astModel` (BaseModel detection). `generateAll` is AST-only; non-statically-analyzable controllers throw (no boot fallback). Content-type request maps + route-level middleware are extracted (gaps closed); only dynamic `routes`/`middleware`/`getHttpPath` defer. **The boot path is deleted** — `importResolution.ts`, `ghostController.ts`, `routeTypes.ts` (boot), `emit.emitGenFile`, and the boot-only tests are gone. Correctness gated by `routeTypes.golden.test.ts` (now runs the AST pipeline + tsc-checks the output) + per-module units. All green: 128 codegen+routing tests. **[BREAKING]** logged: controllers must be declarative. Prototype ✅ 2026-06-07 (`docs/codegen-ast-approach.md`).
**Target**: **v5 final (v5.0.0)** — shipped. ⚠️ The plan below kept the boot/ghost path as a v5 *fallback* (delete in v6); **as built, the boot path was deleted in v5.0.0** and non-declarative controllers are a hard error (one [BREAKING] note). The phase plan is preserved as the route taken; the deletion that was "Phase 7 / v6" happened in v5. See the Status line and `docs/codegen-ast-approach.md` → *Final implementation*.
