...
- **A schema that turns the `id` virtual off, or defines its own `id` path, no longer gets a phantom `id: string` on its documents.** The hydrated document type intersected `{ id: string }` unconditionally — a leftover from before Mongoose added the virtual to its own inference (9.1, refined in 9.4), which now supplies `id: string` itself and honours both a `schemaOptions` `id: false` and a real `id` field. The forced intersection won over both: with `id: false`, `doc.id` was typed `string` while the runtime value is `undefined`; with `id: { type: Number }`, it was `string` while the runtime value is a number. The intersection is gone, so the document follows the schema in both cases and the default case is unchanged (`doc.id` is still `string`). A custom `id` *virtual* is a different matter: it is still intersected with Mongoose's default `id: string`, so one that returns anything other than a string collapses to `never` — declare it as a path or rename it. Type-level only. Pinned by the new `idVirtual.ts` fixture in the model-typing compile gate.

- **A single-nested subdocument spelled `field: { type: { … }, _id: false }` no longer carries a phantom `_id` on hydrated documents.** Mongoose disables the generated `_id` for both spellings — the `_id: false` beside `type:` and the one inside it — but the hydrated-type correction unwrapped the `type:` wrapper before checking for the marker, so the sibling spelling kept an `_id: Types.ObjectId` the runtime never sets, while the inside spelling was handled. The marker now survives the unwrap. The raw and lean surfaces were already correct and are unchanged; a subdocument without `_id: false` keeps its generated `_id` as before. Type-level only. Pinned in the `nestedPathIds.ts` fixture, which now covers both spellings on both surfaces.

- **Virtuals that are not a plain zero-argument getter no longer type as `never`.** The virtual mapper understood only `get()` with no parameters, so a getter declared with Mongoose's `(value, virtual, doc)` arguments, a set-only virtual, and a populate virtual (`{ options: { ref, localField, foreignField } }`, no getter) all resolved to `never` — which is assignable to anything, so wrong code compiled silently. The rules are now: a getter's return type wins whatever its parameters; a set-only virtual takes its setter's value type; a virtual with neither (populate virtuals) is `unknown`, because the populated shape is not knowable from the schema — narrow it, or intersect the document type at the call site. Virtuals declared `as const` stay read-only on the document, as before. Type-level only. Pinned by the new `virtualShapes.ts` fixture.

### Changed

- **Behavior change (types) — a model resolved by runtime name is queryable now, and `AppModel` is a single model type instead of a union.** `AppModel`, the type behind `getModelOrThrow(name)` and behind `getModel()` for the framework's own model names, was the union of the legacy `AbstractModel`-based model type and the `BaseModel`-derived one. TypeScript cannot resolve Mongoose's overloaded finders on such a union, so `find`, `findOne` and `findById` were compile errors on exactly the path meant for names known only at runtime, while `create`, `updateOne`, `deleteMany` and `countDocuments` happened to compile. `AppModel` is now the `BaseModel`-shaped model type; legacy `AbstractModel` models are stored under the same type until v6 removes them. Two things can stop compiling. Because the raw document type is the base shape (`_id` plus timestamps), `create({ … })` with an object literal carrying other fields is now rejected by the excess-property check, where the union let it through — write through a runtime-name model with `new Model(obj)`, a typed value, or a cast to the concrete model type when you know it; `getModel('Name')` with codegen remains the fully typed path. And an `AbstractModel`-based model (`myModel.mongooseModel`) is no longer assignable to `AppModel` — cast at that boundary, as the framework now does internally. Type-level only; runtime resolution is unchanged. Pinned by the `appModelLookup.ts` codegen fixture, which now exercises the finders and asserts the type is not `any`.
...
- **Bun 1.4+ is now a supported runtime.** The same application code runs on Bun through its Node compatibility layer and the existing Express adapter — no Bun-specific build, no `Bun.serve` path, no dependency pin or override. 1.4.0 is the declared floor because it is the first stable Bun that imports the Mongoose/BSON dependency graph the framework resolves; `engines` now declares `bun: >=1.4.0` alongside `node: >=24.0.0`, which npm and Bun both read as alternative runtime constraints. CI runs two new jobs on the floor and on latest stable Bun: the framework suite (`npm run test:bun`) and a packed-consumer smoke test (`npm run smoke:bun`) that installs the published tarball with `bun install`, imports every public entry point, checks the exports map still blocks internal subpaths, boots a `Server` against a real MongoDB, serves a request, runs Mongoose create/read/update/delete and shuts down clean. Node remains the primary runtime and is unaffected.

  Two things stay Node-only, and are documented in `README.md`. **`cluster.js` / `runCluster`** imports fine under Bun but only the Node path is tested — run single-process under Bun. **Node's test-runner CLI**: Bun implements the `node:test` API but not `node --test`, so a Bun project runs its suite with `bun test` plus the framework's existing preloads instead of the Node flags. Four framework test files need `mock.module()` (Node's `--experimental-test-module-mocks`), which Bun does not implement ([oven-sh/bun#5090](https://github.com/oven-sh/bun/issues/5090)); they are the only exclusions from the Bun run and are named in the README. Every other test passes on both runtimes. One further Bun quirk worth knowing when writing tests: `assert.deepStrictEqual` reports any `Proxy` — which is what a Mongoose array is — as unequal, so spread it first (`[...doc.tags]`).
...
- **The shipped email templates are now JavaScript template modules, and the built-in auth mails require `@adaptivestone/framework-module-email` >= 2.1.** `recovery` and `verification` shipped as `.pug` files, which rendered only because the email module used to bundle Pug; v2 dropped every bundled engine, so on module v2 an app that triggered password recovery or email verification without registering a Pug engine got a render failure instead of a mail. Each template file is now a small module whose default export turns the render data into the rendered string — `html`, `subject` and `text` per template folder — which module 2.1 renders with its built-in module engine. `tsc` compiles them into `dist`, and `postbuild` no longer copies `src/services/messaging` wholesale: the email module picks its renderer from the on-disk extension, so a raw `.ts` sitting next to the compiled `.js` could be handed to a consumer that cannot run it. A published template folder now holds compiled `.js` only, which the packaging smoke test asserts and renders. `folders.emails` overrides are unchanged — an app still points at its own folder and ships `.ts` (run natively), `.js` (compiled) or any extension it registers an engine for. The `peerDependencies` range narrows from `^1.0.0 || ^2.0.0` to `^2.1.0` to match: **an older module renders the new defaults as an unknown-extension error, exactly the failure the `.pug` defaults already produced there, so no working setup regresses.** The module stays an *optional* peer — an app that never sends the built-in auth mails still needs none of it. The data a template receives is typed by `TEmailTemplateData` (`locale`, `t`, `link`, `editor`, plus anything `globalVariablesToTemplates` adds), which ships compiled in `dist` but is not on the package `exports` map yet.

- **The shipped email HTML now declares the language it is written in.** `recovery` and `verification` hardcoded `<html lang="en">` while every sentence inside already followed the request locale, so a Russian mail still announced itself as English to screen readers, mail clients and translation prompts. Both templates now render `lang` from the `locale` the email module passes in — escaped like every other interpolated value — and fall back to `en` when it is absent or empty.
...
- **Verification emails were hardcoded Russian.** `verification/{subject,html,text}` carried Russian sentences and not a single `t()` call, so every app sent `Верификаия емейла` to every user whatever their locale — and the HTML greeted `#{username}`, a variable the auth flow never passes, so the greeting rendered empty for everyone. The three templates now mirror `recovery`: English text, every string emitted through `t(key, { defaultValue })` so a key present in your locales wins and a miss reads as English, and the greeting taken from the existing `email.greeating` key instead of a phantom variable.
  ...
  `email.emailConfirm` and `email.verify` were already in the locale files with no emit site anywhere; now that they are emitted their English text lives in code like every other framework message, so the two keys were dropped from `en/translation.json`. `ru/translation.json` still wins for `ru` requests and now carries a Russian `email.verifyInstructions` of its own. Values interpolated into the HTML body are escaped, which the template language used to do implicitly.

- **An auth mail for a user with no email address is now a logged `false` instead of a crash or a mail to nobody.** `email` is optional on both `User` and `UserOld`, but `sendVerificationEmail` and `sendPasswordRecoveryEmail` went straight to minting a token and handing `this.email` to the mailer: `User` rejected with `Email is required` from the token helper it calls first, and `UserOld` hashed the literal string `undefined` into a token, saved it on the user, and then called `mail.send(undefined)`. All four methods now check the address first, report `Cannot send <kind> email: user has no email address` through the same logger that already reports a missing mailer module, and return `false` — the value every call site handles.
...
- **Two more dead framework locale keys.** `auth.userProvided` and `email.newPassword` had no emit site in the framework either. `auth.userProvided` was quoted only by a benchmark fixture, which now carries its English text as an in-code `defaultValue` like every framework message does, so nothing looks the key up any more. Both are gone from `en/translation.json` and `ru/translation.json` — that leaves `en`'s `email` namespace empty, so it went with them. An app that defines the same keys in its own locales is unaffected.
...
- **`oxc-parser` is now an optional peer dependency — action required if you run code generation.** It is a native (napi) package used only by the codegen front end (`astExtract`/`astConfig`), reachable only through the `generatetypes` command, and it was never loaded at runtime — yet every production install carried it and its platform binary (~3 MB, about 11% of the installed footprint). It is now loaded lazily and declared optional, so `npm ci --omit=dev` no longer pulls a native binary the app will never call. **Projects that run `npm run gen` must add it as a devDependency** (`npm i -D oxc-parser`); the command fails with that exact instruction if it is missing. Same treatment already given to `@redis/client` and `yup`, and the packaging smoke test now asserts the package stays absent from a default install.
...
- **Generated config types no longer freeze homogeneous arrays to their current length.** Runtime samples such as one CORS origin, two preload entries, nested numeric lists, or repeated object records now emit reusable `T[]` shapes instead of fixed-length tuples, so ordinary indexing and different environment-specific lengths remain valid. Genuinely heterogeneous arrays still emit tuples to preserve their distinct positional types, and empty arrays remain `unknown[]` because codegen has no element evidence.
- **Generated model lookups now reject misspelled and arbitrary names.** `IApp.getModel()` accepts only framework-owned names plus the application model-name map emitted into `genTypes.d.ts`, so an unknown literal can no longer fall through to a broad `string` signature; a union of valid names resolves to the corresponding union of model types. Runtime-selected names use the new `getModelOrThrow(name)` API, which returns a model without a `false` branch and throws a guided error when the model is unavailable while remaining precise for generated names.
- **Model documents now distinguish raw schema values from hydrated Mongoose values.** `GetModelTypeFromClass` and `GetModelTypeLiteFromSchema` previously reused `InferRawDocType` for both sides, so a hydrated subdocument array looked like a plain array: generated `_id` and defaulted paths were demanded when calling `push()`, while runtime `DocumentArray` methods such as `create()` and `id()` were missing. Hydrated documents now use Mongoose's hydrated schema inference, preserving `DocumentArray`, Mongoose array, and `Map` APIs; `create()`, `push()`, and `splice()` accept values that Mongoose casts, generated subdocument ids remain required when read, and raw/lean values remain plain arrays. Inline `_id: false` is also respected instead of leaking Mongoose's inferred `_id: unknown`, and `DocumentArray` is preserved through nested TypeScript field overrides. Because a hydrated array property is now accurately a `DocumentArray`, replacing it with a native array by direct assignment is intentionally rejected; use in-place `splice(0, array.length, ...values)` or Mongoose `set('path', values)` when replacing its contents. This guarantee requires Mongoose 9.9.1 or newer.
- **Inline parameterized middleware no longer breaks controller inheritance.** Unannotated derived route and controller-middleware getters receive no contextual return type from TypeScript, so `[Middleware, params]` widens from a tuple to an array before override compatibility is checked; this previously raised `TS2416`/`TS2417` unless every entry used `as const` or the whole getter was annotated. The public middleware authoring type now accepts that inferred readonly array form as well as exact tuples, preserving codegen-friendly inferred route objects without per-entry assertions. Runtime normalization still requires exactly two elements, a middleware class first, and a parameter object second.
- **Reusable User auth statics now require only the schema fields they use.** Email lookup no longer requires password or token arrays, password login no longer requires verification/recovery arrays, and each token lookup requires only its own token path. Customized models can therefore reuse a supported auth operation without implementing unrelated flows, while each generic static still returns the caller's concrete document type and its project-added fields.
- **Schema-level Mongoose lean defaults now flow through reduced model authoring types.** `GetModelTypeLiteFromSchema<Schema, Options>` previously omitted Mongoose's `Schema` generic, so a runtime `schemaOptions` value of `lean: true` still made `find()`, `findOne()`, `findById()`, and find-and-modify operations appear to return hydrated documents. The helper now carries its effective literal schema options through a variance-neutral schema carrier: literal and object-form lean defaults return plain objects, while an explicit `{ lean: false }` returns hydrated documents. Complete class-derived models keep their precise schema/method/static surface and remain assignable to reduced `this` contexts. This guarantee requires Mongoose 9.9.1 or newer.
- **Object-form Mongoose timestamp options now produce the paths used at runtime.** Full and Lite model types previously treated every timestamp value other than the literal `false` as the default `{ createdAt, updatedAt }` pair. A model using `timestamps: { createdAt: true, updatedAt: false }` therefore exposed a nonexistent `updatedAt`, while custom timestamp names were absent from its inferred document. Timestamp objects now independently preserve, disable, or rename both paths; an omitted object key keeps its Mongoose default. Lite authoring types for models with non-default options must pass `typeof Model.schemaOptions` as their second argument.
- **Customized User auth statics remain callable with Mongoose 9.8.1.** Mongoose now reconstructs a typed `Model.schema` when the schema generic is left at its `any` default, which made the framework's structural `UserAuthModel` bridge invariant in every raw schema field. Additive and composed project User models therefore failed the `this` constraint on `getUserByEmailAndPassword`, token lookup, and related statics even though they preserve every auth field. The bridge now marks only its unused schema surface as `unknown`; query/update operations and each caller's concrete document return type remain precise, with no consumer casts. Its required auth-field value types are picked from the schema-inferred User document rather than declared separately, keeping `User.modelSchema` as the single source of truth.
- **Readonly model schemas now infer required-message fields and ordinary arrays correctly.** TypeScript's recommended `as const` spelling turns `required: [true, 'message']` and array definitions into readonly tuples, but Mongoose's required-path detector recognizes only their mutable equivalents; those paths therefore gained a false `| null | undefined` in both full and lite framework model types. The framework now removes readonly container/property modifiers at the inference boundary while preserving literal values, explicit `required: false`, and `default: undefined | null`. This is type-only: runtime schemas are unchanged, and arrays explicitly opted out of Mongoose's default `[]` remain nullable.
...
- **`UserOld` legacy auth is hardened, and its deprecation is now audible.** The deprecated `UserOld` model — kept until v6 for consumers who haven't migrated — received three surgical changes. Its constructor deprecation notice is now a `process.emitWarning` with code `ASF_DEP_USEROLD` and an explicit security note (its legacy auth is known-weak; migrate to the `User` model), where it was a bare `console.warn`. The recovery- and verification-token lookups now include `until: { $gt: new Date() }`, so an expired token is rejected — previously an admitted TODO left a stale token valid forever. And the unknown-email login path now performs the same KDF burn as the `User` model (`burnPasswordVerify`), closing an account-enumeration timing oracle that had returned before doing any hashing work on an unknown email. Session-token behavior is unchanged, and only a consumer who deliberately kept authentication on `UserOld` across the v5 migration is affected. **Behavior change (UserOld consumers):** a recovery or verification token past its expiry is now rejected instead of continuing to work indefinitely.

### Changed

- **The development toolchain now uses stable TypeScript 7.0.2.** TypeScript 6 and the separate `@typescript/native-preview`/`tsgo` preview compiler have been replaced by the stable TypeScript 7 package, and all build and type-check scripts now use `tsc`. Codegen syntax tests now invoke the compiler CLI because TypeScript 7 no longer exposes the legacy `transpileModule`, `ModuleKind`, and `ScriptTarget` API from the package root. Runtime behavior and the framework's published API are unchanged.

### Fixed

- **Route-type codegen now accepts middleware tuples wrapped in TypeScript-only expressions such as `as const`.** A route-level tuple like `middleware: [[RateLimiter, options] as const]` is represented by OXC as a `TSAsExpression` around the tuple, but the extractor inspected middleware entries only as direct identifiers or array expressions. It therefore misclassified the otherwise-static route as using unanalyzable middleware and skipped its generated `*.routes.gen.ts` file. Middleware extraction now unwraps `as`, `satisfies`, non-null, and parenthesized expressions around the complete list, each entry, and the binding at the head of a tuple. The same handling applies to tuples in a controller's static middleware `Map`; genuinely dynamic middleware such as `buildMiddleware()` remains rejected.
...
- **A handler on a schema-less route that reads `req.appInfo.request`/`.query` no longer crashes with a logged `500`.** Both fields are typed as non-optional `Record<string, unknown>`, but `PrepareAppInfo` seeded only `{ app }` and the real assignments were gated on schema presence — so on a route with no `request:`/`query:` schema, a handler reading `req.appInfo.query.page` type-checked cleanly yet threw `TypeError` at runtime, which the handler wrapper caught and logged as a `500` "Platform error". `PrepareAppInfo` now seeds both fields to `{}`; a route that declares a schema is unchanged (the validated values still overwrite the defaults). **Behavior change:** such a handler now runs — each absent key reads as `undefined` — instead of `500`ing.

- **Controller load order is now a contractual, consistent sort rather than a coincidence of V8's sort internals.** The index-first comparator returned `-1` when `a` was an index file and `0` otherwise — it never returned `1`, so `compare(index, nonIndex)` was `-1` while `compare(nonIndex, index)` was `0`, violating ECMA-262's antisymmetry requirement. Index-first registration (and root `index` before a nested `index`) therefore held only by sort luck, and a V8 sort-strategy change or a longer controller list could have loaded a nested controller before a root index one, shifting middleware accumulation at boot. A new `compareControllerLoadOrder` is antisymmetric and consistent — index files before others, a shallower index before a deeper one — and separator-agnostic (works with `\` paths). Registration order for existing apps is unchanged.

- **Route-type codegen no longer truncates non-identifier path-param names, and a literal `*/` in a route path no longer breaks the generated file.** (a) `parsePathParams` matched only `:([a-zA-Z_][a-zA-Z0-9_]*)`, while the runtime router takes the whole segment after `:` — so `'/order/:order-id'` emitted `params: { order: string }` even though the runtime populates `req.params['order-id']`, and `:1st` was dropped entirely; codegen now captures the full segment and quotes any key that isn't a valid identifier (`'order-id': string`). (b) A literal `*/` in a route path (a legal static segment — only a *leading* `*` is a splat) terminated the generated file's JSDoc comment and broke parsing; the path is now escaped where it's rendered into the comment. Output for normal routes is byte-identical.

- **The OpenAPI generator no longer emits duplicate query parameters.** `buildQueryParameters` concatenated the route's own query schema with every middleware's introspectable `relatedQueryParameters` and pushed one parameter per property with no dedup — so a route query key that overlapped a middleware-declared one (e.g. `limit` on both) produced two `{ name: 'limit', in: 'query' }` entries, which OpenAPI 3.1 forbids and validators (Redocly, Spectral, swagger-cli) reject. Parameters are now deduplicated by `(name, in)`, with the route's own schema taking precedence over middleware contributions. A document that had no collision is byte-identical.

- **A config or model name containing a quote or backslash no longer breaks the generated `genTypes.d.ts`.** Config names, model names, and model paths were interpolated into single-quoted TypeScript string literals with no escaping, while the `sq()` escaper lived only in `emit.ts` — so a name that is legal on disk but contains `'` or `\` (e.g. `config/user's.ts`) produced an unterminated string literal that broke the whole project's typecheck. Every interpolation site now escapes through the shared `sq()`; output for benign names is byte-identical.
