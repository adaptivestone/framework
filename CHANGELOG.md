...
- **[NEW]** Route matching options `config.http.routing.caseSensitive` / `strictTrailingSlash` (default lenient) are wired through to the adapter — the matcher already supported them but they weren't configurable. Clustered deployments: the primary forwards `SIGTERM`/`SIGINT` to workers (each drains via its own handler) and exits once they're gone.
...
- **[FIX]** Router static-child lookup is now **O(1)** instead of a per-request linear scan. The tree matcher's `lookupStaticChild` walked every static child of every node on the path with a `toLowerCase()` allocation per child — and since `caseSensitive` is always `false` in production, that scan was the *only* path requests took (the O(1) branch was dead). Static children are now keyed by lowercase segment at registration time, so the default case-insensitive match is a single `Map.get`; the case-sensitive path does the same get plus an exact-segment check. Nodes keep their original-cased `segment` (display / `formatTree` / codegen paths unchanged — gen output byte-identical). Segments differing only by case now fold onto one node (case-insensitive matching already treated them as one); a same-method collision surfaces via the existing conflicting-handler error.
