...
describe('astEmit', () => {
  testEach(
    ...
    async (name) => {
      ...
      assert.strictEqual(resolved.needsBoot, false);
    },
  );
...
export default class Legacy extends Base {
  ...
  static get middleware() { return new Map([['/{*splat}', [Mw]]]); }
      ...
      assert.deepStrictEqual(needsBoot, []);
      ...
      assert.ok(gen.includes('AUTOGENERATED'));
      assert.ok(gen.includes("import type Mw from './Mw.js';"));
    } finally {
  ...
  get routes() { return { get: { '/': this.list } }; }
      ...
      assert.deepStrictEqual(needsBoot, []); // the .d.ts files were NOT discovered
      assert.strictEqual(written, 1); // only Widget.ts
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  }, 30_000);
