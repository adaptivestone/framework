...
class Auth extends AbstractController {
    ...
    async postLogin(req, res, next) {
        ...
        let user = await User.getUserByEmailAndPassword(req.body.email, req.body.password);
        if (!user) {
            return res.status(400).json({error: req.i18n.t('auth.errorUPValid')});
        }
        if (!user.isVerified) {
            return res.status(400).json({error: req.i18n.t('email.notVerified'), notVerified: true});
        }
        let token = await user.generateToken();

        return res.status(200).json({success: true, token: token, user: user.getPublic()});
    }
    ...
    async postRegister(req, res, next) {
        ...
        if (!req.body.email) {
            errors.email = [req.i18n.t('auth.emailProvided')];
        } else if (!validator.isEmail(req.body.email,{allow_utf8_local_part:false})) {
    ...
    async verifyUser(req, res, next) {
        ...
        let user;
        try {
            user = await User.getUserByVerificationToken(req.query.verification_token);
        }
        catch (e) {
            return res.status(400).json({success: false, error: req.i18n.t("email.alreadyVerifiedOrWrongToken")});
        }
    ...
    async sendPasswordRecoveryEmail(req, res, next){
        ...
        try {
            ...
            if(!user){
                return res.status(400).json({success: false, error: req.i18n.t("auth.errorUExist")});
            }
    ...
    async recoverPassword(req, res, next){
        ...
        try {
            user = await User.getUserByPasswordRecoveryToken(req.query.password_recovery_token);
        }
        catch (e) {
            return res.status(400).json({success: false, error: req.i18n.t("password.wrongToken")});
        }
